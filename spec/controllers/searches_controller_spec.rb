  ...
  describe 'POST "results"' do
    context 'Twitter search' do
      it 'displays results page with valid provider' do
        post :results, {username: "Bookis", provider: "Twitter"}, {user_id: "123"}
        expect(response.status).to eq 200
      end
      ...
      it 'does not display results page with invalid provider' do
        post :results, {username: "Bookis", provider: "foobar"}, {user_id: "123"}
        expect(response).to redirect_to(search_path)
      end

      it 'does not display results page without provider' do
        post :results, {username: "Bookis"}, {user_id: "123"}
        expect(response).to redirect_to(search_path)
      end

      it 'catches no search query' do
        post :results, {username: "", provider: "Twitter"}, {user_id: "123"}
        expect(response).to redirect_to(search_path)
      end

      it 'sets @bookis if no results found' do
        post :results, {username: "aawegawg920j2vn8g2479913sj0x", provider: "Twitter"}, {user_id: "123"}
